...
def make_group_result(keep: list[str], drop: list[str]) -> dict[str, Any]:
...
def allocate_removal_budget(
    ...
    Larger groups receive more of the removal budget. Any deficit from integer rounding is distributed to the largest
    groups first; any surplus is trimmed from the largest groups first.
...
def weighted_sorting(
    samples: NDArray[np.object_], weights: NDArray[np.float64], *, sort_ascending: bool = True
) -> tuple[NDArray[np.object_], NDArray[np.float64]]:
    """Sorts the samples of an array based on their weight values.
...
def weighted_sorting_gumbel(
    ...
    *,
    ...
    # Calculate the exponent term (1 / W_i); if the weight of a sample is low its exponent will be high.
...
def greedy_select_from_sim_matrix(
    ...
    r"""Greedy submodular selection given a precomputed (N x N) similarity matrix.

    Iteratively selects samples that minimise:
        P(S_j) = sum_{i in C_k} sim(i, j)  -  sum_{i in D_k \ C_k} sim(i, j)
    ...
    # selected_sim[j] = sum_{i in C_k} sim(i, j) - starts at zero (C_k is empty).
    selected_sim = np.zeros(num_scenarios, dtype=np.float64)

    # unselected_sim[j] = sum_{i in D_k\C_k} sim(i, j) - initially all are unselected.
