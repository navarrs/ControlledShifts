...
from controlledshifts.utils.constants import EPSILON, LARGE_FLOAT
...
def compute_cosine_similarity(samples: NDArray[np.float64], target: NDArray[np.float64]) -> NDArray[np.float64]:
    ...
    Args:
        ...
        target: array of shape (embedding_dim,).

    Returns:
        similarities: per-sample cosine similarities in [0, 1].
    """
...
def compute_jaccard_index(a: set[int], b: set[int]) -> float:
    ...
    Returns:
        jaccard_index: the intersection over union value [0, 1].
    """
...
def compute_hamming_distance(a: list[int], b: list[int], *, return_inverse: bool = False) -> float:
    ...
    Args:
        a: first categorical vector.
        b: second categorical vector.
        return_inverse: if True, returns 1 - distance instead.

    Returns:
        Fraction of mismatched positions (or its inverse when return_inverse). Raises ValueError if lengths differ.
    """
    if len(a) != len(b):
