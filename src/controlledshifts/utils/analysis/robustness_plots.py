...
def _abbrev_caption(metrics: list[str]) -> str:
    ...
    return "   ·   ".join(
        f"{mathtext_bold(metric_abbrev(metric))} = {metric_label(metric)}"
...
def _abbrev_key_entries(metrics: list[str]) -> tuple[list[Patch], list[str]]:
    ...
    return [Patch(label=metric_abbrev(metric)) for metric in keyed], [f"= {metric_label(metric)}" for metric in keyed]
...
def _add_summary_legend(  # noqa: PLR0913
    fig: plt.Figure, handles: list, labels: list[str], metrics: list[str], legend_left: float, legend_right: float
) -> None:
    """Attach the shared model legend, with the radar rim abbreviations as a bold second row of the same box.

    Stretched across the full width of the ranking panels (edges tracked by the caller); ``mode="expand"`` fills the
    bbox so entries spread evenly rather than clumping in the center.
    """
...
def plot_combined_summary(
    ...
    for col, mode_result in enumerate(mode_results):
        handles, labels, column_axes, (panel_left, panel_right) = _draw_summary_column(
            fig, gs, col, mode_result, palette, colormap
        )
        if col == 0:
    ...
    _add_summary_legend(fig, handles, labels, metrics, legend_left, legend_right)
