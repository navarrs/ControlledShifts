...
background_agents benchmark additionally writes its perturbed variant stores under ``variants_path`` as a
preparation step.
...
Example usage:
    ...
    # Background Agents benchmark (generates all perturbation variant stores up front)
    uv run -m controlledshifts.create_benchmark benchmark=background_agents

    # Background Agents Hard benchmark
...
def _compute_split(benchmark: Benchmark, cfg: DictConfig) -> BenchmarkSplit:
    ...
    match benchmark:
        ...
        case Benchmark.BACKGROUND_AGENTS:
            return benchmarks.create_background_agents_benchmark(cfg)
        case Benchmark.BACKGROUND_AGENTS_HARD:
            return benchmarks.create_background_agents_hard_benchmark(cfg)
...
def main(cfg: DictConfig) -> None:
    ...
    otherwise the benchmark computes the split, runs any data preparation (such as background_agents' perturbations
